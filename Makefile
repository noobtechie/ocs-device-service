...
MICROSERVICES=cmd/device-ocs/device-ocs
.PHONY: $(MICROSERVICES)
...
GOFLAGS=-ldflags "-X github.com/edgexfoundry/device-ocs.Version=$(VERSION) \
...
cmd/device-ocs/device-ocs:
	go mod tidy
	CGO_ENABLED=0  go build $(GOFLAGS) -o $@ ./cmd/device-ocs

docker:
	docker build \
		-f cmd/device-ocs/Dockerfile \
		--label "git_sha=$(GIT_SHA)" \
		-t edgexfoundry/device-ocs:$(GIT_SHA) \
		-t edgexfoundry/device-ocs:$(DOCKER_TAG) \
		.
